    ...
    return new Promise(resolve => {
      function tick(now) {
        ...
        if (phase === 'paint') {
          ...
          // Cap the per-frame step so a dropped frame (e.g. while the page is
          // scrolling) advances the writing in a smooth chunk rather than a big
          // catch-up jump — keeps the stroke speed visually steady under load.
          const dt = Math.min(now - lastWallT, 50);
          lastWallT = now;
          virtualTime += dt * rate;
