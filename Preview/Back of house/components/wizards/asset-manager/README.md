...
| `AssetManagerStep5AssetBuilder.jsx` | 5 · Asset builder | Stencils/stickers/backgrounds libraries (premade + the client's own creations in one multi-select grid), plus a guided Upload → Adjust → Review maker that converts any image client-side: stencils to 1024×1024 PNGs with a transparent paintable cut-out (threshold + invert), stickers to 1024×1024 transparent PNGs with auto background removal, white die-cut padding, and micro shadow, backgrounds cover-cropped to 16:9. "Make with AI" is a placeholder panel. |
| `AssetManagerStep5AssetBuilder.BACKEND.md` | — | Backend integration spec for Step 5: asset format contract, `onChange` payload + data-URL persistence, hydration, the future AI generation hook, server-side validation, suggested data model. |
| `AssetManagerStep5AssetBuilder.demo.html` | — | Standalone, zero-build preview of Step 5 alone — open directly in a browser; the full upload-and-convert pipeline works without a backend. Not needed for integration. |
